...
class ParNetEncoder(nn.Module):
    def __init__(self, in_channels, block_size, depth) -> None:
        ...
        self.depth = depth
        self.d_pre1 = Downsample(self.in_channels, self.block_size[0])
        self.d1 = Downsample(self.block_size[0], self.block_size[1])
        self.d2 = Downsample(self.block_size[1], self.block_size[2])
        self.d3 = Downsample(self.block_size[2], self.block_size[3])
        self.d4 = Downsample(self.block_size[3], self.block_size[4])
        # self.d5 = Downsample(self.block_size[3], self.block_size[4])
        self.stream1 = nn.Sequential(
            *[Stream(self.block_size[2], self.block_size[2]) for _ in range(self.depth[0])]
        )

        # self.stream1_downsample = Downsample(self.block_size[1], self.block_size[2])

        self.stream2 = nn.Sequential(
            *[Stream(self.block_size[3], self.block_size[3]) for _ in range(self.depth[1])]
        )

        self.stream3 = nn.Sequential(
            *[Stream(self.block_size[4], self.block_size[4]) for _ in range(self.depth[2])]
        )
...
class ParNet(BaseModel_backbone):
    def __init__(self,
                 in_channels=3,
...
def parnet_sm():
    return ParNet(in_channels=3, block_size=[32, 64, 96, 192, 384])
