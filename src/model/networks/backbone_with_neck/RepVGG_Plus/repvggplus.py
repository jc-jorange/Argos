...
class RepVGGplus(BaseModel_backbone_with_neck):
    """RepVGGplus
    ...
    def __init__(self,
        ...
        self.inplanes = stage_channels[3]
    ...
    def _build_aux_for_stage(self, stage):
        ...
        return nn.Sequential(downsample, nn.AdaptiveAvgPool2d(1), nn.Flatten())
...
class SEBlock(nn.Module):
    ...
    def forward(self, inputs):
        x = F.avg_poo2d(inputs, kernel_size=inputs.size(2))
        x = self.down(x)
        x = F.relu(x)
        x = self.up(x)
        x = torch.sigmoid(x)
