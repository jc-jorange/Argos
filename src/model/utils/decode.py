...
from src.model.utils import _gather_feat, _tranpose_and_gather_feat
...
def mot_decode(heatmap,
    ...
    heatmap = _max_pool(heatmap)  # default use 3×3max pooling, target number become 1/9 of feature map
    ...
    if cat_spec_wh:
        ...
        wh = wh.gather(2, clses_ind).view(N, K, 4)
    else:
        wh = wh.view(N, K, 4)

    classes = classes.view(N, K, 1).float()
    scores = scores.view(N, K, 1)
    bboxes = torch.cat([xs - wh[..., 0:1],   # left    x1
                        ys - wh[..., 1:2],   # top     y1
                        xs + wh[..., 2:3],   # right   x2
                        ys + wh[..., 3:4]],  # down    y2
                       dim=2)
